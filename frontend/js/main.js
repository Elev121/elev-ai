...
(function initCharts() {
  ...
  const datasets = {
    ELEV:         { data: [92, 89, 96, 88, 95, 91], color: '#3b82f6' },
    'Manual Review':{ data: [78, 82, 71, 80, 22, 71], color: '#8b5cf6' },
    'Turnitin':   { data: [55, 60, 94, 48, 85, 65],  color: '#14b8a6' },
    'iThenticate':{ data: [58, 65, 91, 52, 80, 67],  color: '#f97316' },
  };
  ...
  const chartDefaults = {
    ...
    plugins: {
      legend: {
        labels: { color: '#8a9dc0', font: { family: 'Inter', size: 12 }, padding: 16 },
      },
      tooltip: {
        ...
        titleColor: '#f0f4ff',
        bodyColor: '#8a9dc0',
        callbacks: { label: (ctx) => ` ${ctx.dataset.label}: ${ctx.parsed.r ?? ctx.parsed.y}%` },
      },
    },
  };
  ...
  if (radarCtx) {
    new Chart(radarCtx, {
      ...
      data: {
        labels: categories,
        datasets: Object.entries(datasets).map(([label, { data, color }]) => ({
          label,
          data,
          borderColor: color,
      ...
      options: {
        ...
        scales: {
          r: {
            min: 0, max: 100,
            ticks: { stepSize: 20, color: '#4a607e', backdropColor: 'transparent', font: { size: 11 } },
            grid:     { color: '#1e2d4a' },
            pointLabels: { color: '#8a9dc0', font: { family: 'Inter', size: 11 } },
  ...
  if (barCtx) {
    new Chart(barCtx, {
      ...
      options: {
        ...
        scales: {
          x: {
            ticks: { color: '#8a9dc0', font: { size: 11 } },
            grid:  { color: '#1e2d4a' },
          },
          y: {
            min: 0, max: 100,
            ticks: { color: '#8a9dc0', callback: (v) => v + '%', font: { size: 11 } },
            grid:  { color: '#1e2d4a' },
          },
        },
      },
